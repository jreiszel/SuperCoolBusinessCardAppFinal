...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
	...
	public Carousel(Context context, AttributeSet attrs, int defStyle) {
		...
		CarouselImageAdapter adapter = new CarouselImageAdapter(getContext());
	 ...
	 * @date 2015-11-19
	 */
	public void setCarouselAdapter(BaseAdapter baseAdapter)
	{
		this.setAdapter(baseAdapter);
		int selectedItem=0;
	    if(selectedItem < 0 || selectedItem >= baseAdapter.getCount())
	    	selectedItem = 0;
...
//    }
    ...
    private void Calculate3DPosition(CarouselItem child, int diameter, float angleOffset){
    	...
    	float z = r-(float)(r* Math.cos(angleOffset));
    	float y = - getHeight()/2+child.getHeight()/2-r*(float)Math.sin(angleOffset);
